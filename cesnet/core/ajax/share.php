		...
		case 'getShareWith':
			if (isset($_GET['search'])) {
				...
				if ($sharingAutocompletion !== 'yes') {
					...
					// Get user's addressbook contacts matches
					$contactManager = \OC::$server->getContactsManager();
					$addressBookContacts = $contactManager->search($_GET['search'], ['FN', 'EMAIL']);
					foreach($addressBookContacts as $contact) {
						foreach ($contact['EMAIL'] as $email) {
							$usersByMail = OC_User::getDisplayNames((string)$email, 15, 0);
							foreach ($usersByMail as $uid => $displayName) {
								if (in_array($uid, $sharedUsers)) {
									continue;
								}
								if ((!isset($_GET['itemShares'])
